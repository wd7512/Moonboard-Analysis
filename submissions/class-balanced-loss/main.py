"""submissions/class-balanced-loss — Class-Balanced Loss for Moonboard grade prediction.

Based on fast-mlp. Swaps CrossEntropyLoss for ClassBalancedLoss (Cui et al. CVPR 2019)
with effective number of samples weighting: (1-β)/(1-β^n_y).
Sweeps β ∈ {0.9, 0.99, 0.999}. Everything else identical to fast-mlp:
...
Usage:
    uv run python submissions/class-balanced-loss/main.py --help
    uv run python submissions/class-balanced-loss/main.py \
...
# Beta for class-balanced loss — sweeps {0.9, 0.99, 0.999}
# Edit this between benchmark runs.
BETA = 0.999
