"""submissions/coral-engineered-gamma1 — Focal ordinal ensemble with gamma=1.0."""
...
class CORALModel(nn.Module):
    ...
    def forward(self, x):
        h = self.net(x)
        return self.coral_weight(h) + self.coral_bias.unsqueeze(0)
...
def _train_ordinal(model, train_loader, eval_loader, lr, dev, epochs, patience, gamma):
    ...
    for epoch in range(epochs):
        ...
        model.eval(); eval_loss = 0.0; n_batches = 0
        with torch.no_grad():
            for xb, yb in eval_loader:
                xb, yb = xb.to(dev), yb.to(dev)
                eval_loss += criterion(model(xb), yb).item(); n_batches += 1
        eval_loss /= max(n_batches, 1); scheduler.step(eval_loss)
...
def train_and_evaluate(sequences, grades, train_idx, test_idx, seed=42):
    ...
    with torch.no_grad():
        logits_a = model_a(X_test_t.to(dev)); logits_b = model_b(X_test_t.to(dev))
...
def main():
    ...
    if not Path(data_path).exists(): print(f"Error: Data file not found"); sys.exit(1)
    ...
    for seq in all_sequences:
        grade = seq[-2]
        if grade in grade_to_idx: valid_seqs.append(seq); valid_grades.append(grade_to_idx[grade])
