"""submissions/coral-engineered-gamma3 — Focal ordinal ensemble with gamma=3.0."""
...
GAMMA = 3.0  # Higher gamma = more focus on hard examples
...
def main() -> None:
    ...
    if not Path(data_path).exists():
        print(f"Error: Data file not found at '{data_path}'"); sys.exit(1)
    output_dir = Path(args.output_dir); output_dir.mkdir(parents=True, exist_ok=True)
    print(f"Loading data from {data_path}, gamma={GAMMA}")
    ...
    print(f"\nMacro-F1: {results['macro_f1']:.4f}  Exact: {results['exact_accuracy']:.4f}  Time: {elapsed:.1f}s")
