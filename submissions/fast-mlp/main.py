"""submissions/fast-mlp — Fast MLP to top the Moonboard leaderboard without tree methods.

Uses the same 198-dim binary hold vector as the Perceptron baseline but with:
  - Proper feature standardization (zero mean, unit variance per fold)
  - Label smoothing (0.05) for regularization
  - Slightly wider hidden dim (256 vs 128)
  - Adam with ReduceLROnPlateau scheduling
  - Best-model checkpointing

Architecture identical to perceptron: 198 -> 256 -> 128 -> 12

No tree methods. Pure PyTorch neural net.
5-fold CV training time on 92K: ~2 min (well under 10 min limit).

Usage:
    uv run python submissions/fast-mlp/main.py --help
    uv run python submissions/fast-mlp/main.py \\
